...
Write-Step "Creating or reusing rollback checkpoint"
$BackupBase = Join-Path $FlutterProjectPath ".orbit-backups"
$ExistingBaseline = Get-ChildItem $BackupBase -Directory -Filter "ui-runtime-polish-*" -ErrorAction SilentlyContinue |
    Sort-Object Name |
...
if ($null -ne $ExistingBaseline) {
    $BackupRoot = $ExistingBaseline.FullName
    Write-Host "Reusing original UI polish baseline: $BackupRoot"
} else {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $BackupRoot = Join-Path $BackupBase "ui-runtime-polish-$stamp"
...
Write-Host "`nOrbit Flutter UI Runtime Polish v2 installed." -ForegroundColor Green
Write-Host "Run: .\tool\verify-ui-runtime-polish-v2.ps1 -FlutterProjectPath `"$FlutterProjectPath`""
