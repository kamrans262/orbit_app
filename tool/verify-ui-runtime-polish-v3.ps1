...
Require-Contains $camera "final OrbitCircle activeCircle = circle;"
Require-Contains $camera "await _finishVideo(activeCircle);"
Require-Contains $camera "Future<void> _finishVideo(OrbitCircle circle) async"

$cameraText = Get-Content $camera -Raw
$finishVideoMatch = [regex]::Match(
    $cameraText,
    'Future<void> _finishVideo\(OrbitCircle circle\) async \{(?<body>.*?)\n  \}',
    [System.Text.RegularExpressions.RegexOptions]::Singleline
)
if (-not $finishVideoMatch.Success) {
    throw "Could not inspect _finishVideo camera contract."
}
if ($finishVideoMatch.Groups['body'].Value.Contains('activeCircle')) {
    throw "Camera regression: _finishVideo references activeCircle outside _capture scope."
}
if (-not $finishVideoMatch.Groups['body'].Value.Contains('circleId: circle.id')) {
    throw "Camera regression: _finishVideo must use its non-null circle parameter."
}
...
Write-Host "`nOrbit Flutter UI Runtime Polish v3 verification passed." -ForegroundColor Green
